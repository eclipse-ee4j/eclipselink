...
/**
 * This {@link ExpressionVisitor} visits an
 * {@link org.eclipse.jpa.query.parser.Expression JPQL Expression} and creates
 * an {@link org.eclipse.persistence.expressions.Expression Expression}.
 * 
 * @version 2.4
 * @since 2.3
...
final class ExpressionBuilderVisitor implements EclipseLinkExpressionVisitor {
    ...
    /**
     * This visitor creates a list by retrieving either the single child or the
     * children of the {@link CollectionExpression}, which would be the child.
     */
    private ChildrenExpressionVisitor childrenExpressionVisitor;

    /**
     * This visitor is used to create a modified version of a
     * {@link PathExpressionResolver} that does not wrap the last path, which is
     * the collection-valued property name.
     */
    private EmptyCollectionComparisonExpressionVisitor emptyCollectionComparisonExpressionVisitor;

    /**
     * This builder visits the {@link InExpression} in order to create the
     * EclipseLink {@link Expression}.
     */
    ...
    /**
     * This will tell {@link #visit(IdentificationVariable)} to create a
     * {@link ConstantExpression} if this is part of a comparison expression.
     */
    private boolean typeExpression;

    /**
     * The visitor responsible to create the {@link Expression Expressions} for
     * the <b>WHEN</b> and <b>THEN</b> expressions.
     */
     ...
     * 
     * @param queryContext
     *            The context used to query information about the application
    ...
    private void appendJoinVariables(org.eclipse.persistence.jpa.jpql.parser.Expression expression, ReportQuery subquery) {
    ...
    private Set<String> collectOuterIdentificationVariables() {
        ...
        // Now remove the local identification variables that are defined in
        // JOIN expressions and
    ...
    public void visit(AbsExpression expression) {
        ...
        // Note: The type will be calculated when traversing the ABS's
        // expression
    }
    ...
    public void visit(CollectionMemberExpression expression) {
        ...
        if (expression.hasNot()) {
            ...
            // Retrieve the ExpressionBuilder from the collection-valued path
            // expression's
            // identification variable and the variable name
            String variableName = queryContext.literal(expression.getCollectionValuedPathExpression(), PATH_EXPRESSION_IDENTIFICATION_VARIABLE);
    ...
    public void visit(ExistsExpression expression) {
        ...
        // Replace the SELECT clause of the exists subquery by SELECT 1 to avoid
        // problems with
        // databases not supporting multiple columns in the subquery SELECT
        // clause in SQL. The
        // original select clause expressions might include relationship
        // navigations which should
        // result in FK joins in the generated SQL, e.g. ... EXISTS (SELECT
        // o.customer FROM Order
        // o ...). Add the select clause expressions as non fetch join
        // attributes to the ReportQuery
    ...
    public void visit(IdentificationVariable expression) {
        ...
        // The identification variable is virtual, only do something if it
        // falsely represents a state
        ...
        if (typeExpression) {
            ...
            if (queryContext.isResultVariable(variableName)) {
                ...
                if (queryExpression == null) {
                    ...
                    // null would most likely mean it's coming from a state
                    // field
                    ...
                    if (declaration != null) {
                        ...
                        // Cache the identification variable information to
                        // prevent recreating the Expression
    ...
    public void visit(ResultVariable expression) {
        ...
        // Note: The type will be calculated when traversing the select
        // expression
    }
    ...
    public void visit(TypeExpression expression) {
        ...
        // This will tell visit(IdentificationVariable) to create a
        // ConstantExpression
    ...
    private class ChildrenExpressionVisitor extends AnonymousExpressionVisitor {
        ...
        /**
         * The list of
         * {@link org.eclipse.persistence.jpa.query.parser.Expression} that are
    ...
    /**
     * This is used to traverse the empty collection's collection-valued path
     * expression in order to create the right EclipseLink {@link Expression}.
     */
    private class EmptyCollectionComparisonExpressionVisitor extends AbstractExpressionVisitor {
        ...
        public void visit(CollectionValuedPathExpression expression) {
            ...
            // Start at 1 since the first path is always an identification
            // variable, and
            // its resolver was created by traversing it
            for (int index = 0, count = expression.pathSize(); index < count; index++) {

                // The first path is the identification variable, lets
                ...
                // Any path between the identification variable and the last
                // path (which is a
                // state field path expression) is always a single valued object
                // field path
    ...
    /**
     * This {@link Comparator} compares two {@link Class} values and returned
     * the appropriate numeric type that takes precedence.
     */
    ...
    private class PathResolver extends AbstractEclipseLinkExpressionVisitor {
        ...
        public void visit(IdentificationVariable expression) {
            ...
            // It is possible the Expression is null, it happens when the path
        ...
        private void visitPathExpression(AbstractPathExpression expression) {
            ...
            for (int index = expression.hasVirtualIdentificationVariable() ? 0 : 1, count = length; index < count; index++) {
                ...
                if (mapping != null) {
                    ...
                    // Retrieve the reference descriptor so we can continue
                    // traversing the path
                    if (!last) {
                ...
                // No mapping is defined for the single path, check for a query
                // key
                else {
    ...
    /**
     * This visitor is responsible to create the {@link Expression Expressions}
     * for the <b>WHEN</b> and <b>THEN</b> expressions.
     */
    private class WhenClauseExpressionVisitor extends AbstractExpressionVisitor {
        ...
        /**
         * The map of <b>WHEN</b> expressions mapped to their associated
         * <b>THEN</b> expression.
         */
