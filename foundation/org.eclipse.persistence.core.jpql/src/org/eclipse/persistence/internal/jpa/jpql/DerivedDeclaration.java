...
/**
 * This <code>DerivedDeclaration</code> represents an identification variable
 * declaration that was declared in the <code><b>FROM</b></code> clause of a
 * <code><b>SELECT</b></code> subquery. The "root" object is not an entity name
 * but a derived path expression.
 * 
...
final class DerivedDeclaration extends AbstractRangeDeclaration {
    ...
    Expression buildQueryExpression() {
        ...
        // If the derived expression is a class (non direct collection), then
        // create a new expression builder for it,
        // this builder will be compared as equal to the outer reference.
        if ((mapping != null) && mapping.getReferenceDescriptor() != null) {
            ...
            // Otherwise it will be derived by duplicating the join to the outer
            // builder inside the sub select.
            // The same could be done for direct collection, but difficult to
            // create a builder on a table.
            ...
            // Create the local ExpressionBuilder for the super identification
            // variable
