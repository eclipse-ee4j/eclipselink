 ...
 * <p>
 * By default, EclipseLink uses {@link ANTLRQueryBuilder} for parsing a query and converting it into
 * a {@link DatabaseQuery}.
 * <p>
 * Third-parties can implement this interface and provide a different JPQL parsing system if required.
 * The {@link JPAQueryBuilder} implementing class can be registered via a persistence unit property
 * and the implementation class must have a public, zero-arg constructor.
 *
 * @version 2.4
 * @since 2.2
 * @author John Bracken
...
public interface JPAQueryBuilder {
   ...
   /**
    * Creates a fully initialized {@link DatabaseQuery} by parsing the given JPQL query.
    *
    * @param jpqlQuery A non-<code>null</code> string representation of the query to parse and to
    * convert into a {@link DatabaseQuery}
    * @param query The query to populate with the derived JPQL query
   ...
   /**
    * Creates a new {@link Expression} that represents the given selection criteria.
    *
    * @param entityName The name of the entity for which a criteria is created
    * @param selectionCriteria The string representation of a conditional expression to parse
    * @param session The EclipseLink {@link AbstractSession} that this query will execute against
