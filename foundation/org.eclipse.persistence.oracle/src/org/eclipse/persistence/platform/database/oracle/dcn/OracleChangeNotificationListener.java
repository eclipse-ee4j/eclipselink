 ...
 */
public class OracleChangeNotificationListener implements DatabaseEventNotificationListener {
    ...
    public void register(Session session) {
        ...
        for (ClassDescriptor descriptor : session.getDescriptors().values()) {
            ...
            // TODO config
            //if (descriptor.getCacheInvalidationPolicy().isDatabaseEventInvalidationPolicy()) {
                this.descriptorsByTable.put(descriptor.getTables().get(0), descriptor);
            }
        }
        // TODO ensure accessor is connected.
        OracleConnection connection = (OracleConnection)databaseSession.getServerPlatform().unwrapConnection(databaseSession.getAccessor().getConnection());
        ...
        // Register with descriptor events, and add ROWID.
        // TODO use callbacks instead of events.
        for (ClassDescriptor each : this.descriptorsByTable.values()) {
            final ClassDescriptor descriptor = each;
            ...
            if (existingIndex == null) {
                if (descriptor.isChildDescriptor()) {
                    // TODO move to desc init
                    existingIndex = descriptor.getInheritancePolicy().getParentDescriptor().getCachePolicy().getCacheIndex(fields);
                }
                if (existingIndex == null) {
                    existingIndex = new CacheIndex(ROWID);
            ...
            // Check if already been configured (re-login, re-deployment).
            if (!descriptor.getFields().contains(rowId)) {
                ...
                sqlStatement.normalize(databaseSession, null);
                
                final ValueReadQuery transactionIdQuery = new ValueReadQuery("SELECT DBMS_TRANSACTION.LOCAL_TRANSACTION_ID FROM DUAL");
