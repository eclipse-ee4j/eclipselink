...
public class PackageRenamer {
	...
	public boolean isExtensionSupported(String extension) {
		return extension.equalsIgnoreCase("java") || extension.equalsIgnoreCase("xml");
	}
	...
	public void runSearchAndReplacePackageName(File sourceFile) {
		...
		String destinationFileName = destinationRootDir.toString()
				+ sourceFile.separator + destinationFileNameWithoutRoot;
		...
		if (sourceFileNameWithoutRoot.lastIndexOf('\\') >= 0){
			sourceFileNameWithoutRoot = sourceFileNameWithoutRoot.substring(0, sourceFileNameWithoutRoot.lastIndexOf('\\'));
			destinationFileNameWithoutRoot = destinationFileNameWithoutRoot.substring(0, destinationFileNameWithoutRoot.lastIndexOf('\\'));
		}
				...
				if (folder.exists() && folder.isDirectory()) {
					...
					if (file.exists()) {
						...
						throw new IllegalArgumentException(
								"Could not find src to ignore: " + srcRoot
										+ " :: " + packageOrClassName);
					}*/
				}
			} else {
