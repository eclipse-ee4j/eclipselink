...
public abstract class JPQLQueriesTest extends AbstractJPQLTest
{
	...
	public void test_Query_129()
	{
		...
		ExpressionTester addition =
				subExpression(numeric(500)
			.add(
				inputParameter(":int1")));

		ExpressionTester andExpression =
				subExpression(path("p.quantity").greaterThan(addition))
			.and(
	...
	public void test_Query_131()
	{
		...
		ExpressionTester orExpression =
				subExpression(path("p.quantity").lowerThan(numeric(10)))
			.or(
	...
	public void test_Query_133()
	{
		...
		ExpressionTester andExpression =
				subExpression(path("p.quantity").greaterThanOrEqual(numeric(10)))
			.and(
	...
	public void test_Query_137()
	{
		...
		ExpressionTester orExpression =
				subExpression(path("c.home.state").equal(string("'NH'")))
			.or(
	...
	public void test_Query_166()
	{
		...
		(
			...
			where(
					subExpression(path("l.quantity").lowerThan(numeric(2)))
				.and(
					subExpression(
						subExpression(
								path("o.totalPrice")
	...
	public void test_Query_185()
	{
		...
		(
			...
			where(
					subExpression(path("r.amountPaid").multiplication(numeric(".01")))
				.greaterThan(
					numeric("300.00")
				)
			)
	...
	public void test_Query_195()
	{
		...
		(
			...
			where(
				...
				.and(
					variable("cust").memberOf(collectionValuedPath("res.customers"))
				)
			)
		);
