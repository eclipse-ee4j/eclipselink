...
final class RelationalDirectMapGeneralPropertiesPage extends ScrollablePropertiesPage
{
	RelationalDirectMapGeneralPropertiesPage(PropertyValueModel nodeHolder, WorkbenchContextHolder contextHolder)
	{
	...
	protected Component buildPage() {
		...
		helpManager().addTopicID(targetTableWidgets, "mapping.directMap.targetTable");

		// Direct Value Field chooser
		...
		helpManager().addTopicID(directFieldWidgets, "mapping.directMap.directValueField");

		// Direct Key Field chooser
		JComponent directKeyFieldWidgets = 
			buildLabeledComponent(
				"DIRECT_MAP_DIRECT_KEY_FIELD_CHOOSER",
				RelationalMappingComponentFactory.buildDirectKeyColumnChooser(
													getSelectionHolder(), 
													getWorkbenchContextHolder())
		);
		...
		container.add(directKeyFieldWidgets, constraints);
		helpManager().addTopicID(directKeyFieldWidgets, "mapping.directMap.directKeyField");
		...
		JPanel advancedPanel = MappingComponentFactory.buildDirectMapContainerPolicyOptionsBrowser(
			getWorkbenchContextHolder(),
			getSelectionHolder(),
			"mapping.directMap.containerOptions");
		constraints.gridx       = 0;
		constraints.gridy       = 8;
		constraints.gridwidth   = 1;
		constraints.gridheight  = 1;
		...
		addHelpTopicId(container, "mapping.directMap");
		return container;
	}
}
