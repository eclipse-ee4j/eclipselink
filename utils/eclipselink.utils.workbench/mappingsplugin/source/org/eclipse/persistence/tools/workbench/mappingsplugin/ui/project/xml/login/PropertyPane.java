...
public class PropertyPane extends AbstractSubjectPanel
{
	 ...
	 * adding a new {@link PropertyModel} to {@link LoginAdapter}.
	 *
	...
	/**
	 * Create a new <code>ActionListener</code> that will perform the action of
	 * editing the unique selected {@link PropertyModel}.
	 *
			...
			public void actionPerformed(ActionEvent e)
			{
				...
				if (currentWindow() instanceof Dialog)
				{
					...
					(
						...
						(MWProperty) selectionModel.getSelectedValue()
					);
				}
				else
				{
	...
	/**
	 * Create a new <code>ActionListener</code> that will perform the action of
	 * removing the selected {@link PropertyModel}s from {@link LoginAdapter}.
	 *
	...
	private ActionListener buildRemoveActionListener(final ObjectListSelectionModel model)
	{
		return new ActionListener()
		{
			public void actionPerformed(ActionEvent e)
			{
			    ...
			    MWEisProject project = (MWEisProject) subject();
    ...
    */
	...
	private class PropertyEditor extends AbstractValidatingDialog
	{
		...
		/**
		 * This virtual {@link PropertyModel} is used by the {@link PropertyEditor}
		 ...
		 * @param titleKey The key used to retrieve the localized title
		 * @param parentWindow The owner of this dialog
		 * @param property The {@link PropertyModel} to be edited
		 */
		public PropertyEditor(String titleKey,
									 Dialog parentWindow,
									 MWProperty propertyAdapter)
		{
		...
		private void validate(String propertyName)
		{
			...
			// A name is required
			if (virtualProperty.key.length() == 0)
			{
			...
			else
			{
				...
				for (Iterator iter = loginSpec.properties(); iter.hasNext();)
				{
					MWProperty propertyAdapter = (MWProperty) iter.next();

					if ((propertyAdapter != virtualProperty.propertyAdapter) &&
						  propertyAdapter.getKey().equalsIgnoreCase(virtualProperty.key))
					{
		...
		private PropertyValueModel buildNameHolder()
		{
			return new SimplePropertyValueModel()
			{
				...
				public void setValue(Object value)
				{
					...
					PropertyEditorView.this.firePropertyChange(NAME_PROPERTY, oldName, property.key);
				}
			};
		}
	...
	private class VirtualProperty
	{
		...
		/**
		 * The <code>PropertyModel</code> to be edited if not <code>null</code>.
		 */
		 ...
		 * encapsulated.
		 *
		 * @param property The actual <code>PropertyModel</code> to be edited if
		 * not <code>null</code>
		 */
		...
		void apply()
		{
			if (propertyAdapter == null)
			{
			    MWEisProject project = (MWEisProject) subject();
			    MWEisLoginSpec loginSpec = project.getEisLoginSpec();
				propertyAdapter = loginSpec.addProperty(key, value);
			}
			else
			{
				propertyAdapter.setKey(key);
