...
final class XmlReferenceMappingFieldPairEditingDialog
	extends AbstractValidatingDialog
{
	...
	public static void promptToAddFieldPair(
			MWXmlReferenceMapping xmlReferenceMapping, WorkbenchContext context
	) {
		MWXmlFieldPair xmlFieldPair = xmlReferenceMapping.buildEmptyFieldPair();
		FieldPairEditor editor = new FieldPairEditor(xmlFieldPair);
		XmlReferenceMappingFieldPairEditingDialog dialog = 
			new XmlReferenceMappingFieldPairEditingDialog(context, editor);
		
		dialog.show();
		
		if (dialog.wasConfirmed()) {
			editor.commit();
			xmlReferenceMapping.addFieldPair(xmlFieldPair);
		}
	}
	...
	private XmlReferenceMappingFieldPairEditingDialog(
...
//		this.setWarningMessageKey("EIS_REFERENCE_MAPPING_FIELD_PAIR_EDITING_DIALOG.DESCRIPTION");
