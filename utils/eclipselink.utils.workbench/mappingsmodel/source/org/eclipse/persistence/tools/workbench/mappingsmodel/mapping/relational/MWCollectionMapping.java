...
{	
	...
	public static ClassDescriptor legacy50BuildDescriptor() {
		...
		XMLTransformationMapping containerTypeMapping = new XMLTransformationMapping();
		containerTypeMapping.setAttributeName("containerPolicy");
		containerTypeMapping.setAttributeTransformation("legacy50GetContainerPolicyFromRow");
		descriptor.addMapping(containerTypeMapping);
        ...
        XMLTransformationMapping orderingCriteriaMapping = new XMLTransformationMapping();
        orderingCriteriaMapping.setAttributeName("orderings");
        orderingCriteriaMapping.setAttributeTransformation("legacy50GetOrderingsFromRowForTopLink");
        descriptor.addMapping(orderingCriteriaMapping);
        
		return descriptor;
	}
	
	private MWContainerPolicy legacy50GetContainerPolicyFromRow(Record row) {
		MWContainerPolicy cp;
		String containerType = (String) row.get("container-type");
		
		
		if (containerType.equals("Collection")) {
			cp = new MWCollectionContainerPolicy(this);
		}
		else {
			cp = new MWMapContainerPolicy(this);
		}
		...
		SDKFieldValue keyMethodHandleFieldValue = (SDKFieldValue) row.get("key-method");
		Object fieldElement = keyMethodHandleFieldValue.getElements().get(0);
		
		if (fieldElement instanceof Record) {
			Record keyMethodHandleRow = (Record) fieldElement;
			this.legacyValuesMap.put("methodClass", keyMethodHandleRow.get("method-declaring-type"));
			this.legacyValuesMap.put("methodSignature", keyMethodHandleRow.get("method-signature"));
		}
    ...
    private List legacy50GetOrderingsFromRowForTopLink(Record row) {
        List orderings = new Vector();
        SDKFieldValue orderingCriteria = (SDKFieldValue) row.get("ordering-criteria");
       
        String usesOrdering = (String) ((Record) orderingCriteria.getElements().get(0)).get("use-ordering");
        
        if (usesOrdering.equals("true")) {
            String ascending = (String) ((Record) orderingCriteria.getElements().get(0)).get("ascending");           
            String queryKeyName = (String) ((Record) orderingCriteria.getElements().get(0)).get("query-key-name");
            
            this.legacyValuesMap.put("ordering.queryKeyName", queryKeyName);
            this.legacyValuesMap.put("ordering.ascending", ascending);
        }
	...
	public void legacy50PostPostProjectBuild() {
        ...
        if (containerTypeString != null) {//will be null if usesDefaultContainerClass is true
            ...
            if (getInstanceVariable().isAssignableToMap()) {
                this.containerPolicy = new MWMapContainerPolicy(this);
            }
		...
		String methodClassString = (String) this.legacyValuesMap.remove("methodClass");
		String methodSignature = (String) this.legacyValuesMap.remove("methodSignature");
		if (!StringTools.stringIsEmpty(methodClassString) && this.containerPolicy instanceof MWMapContainerPolicy) {
			((MWMapContainerPolicy) getContainerPolicy()).legacySetKeyMethod(methodClassString, methodSignature);
		}	
		
        legacyResolveOrdering();
	}

    private void legacyResolveOrdering() {
        String queryKeyName = (String) this.legacyValuesMap.remove("ordering.queryKeyName");
        String ascending = (String) this.legacyValuesMap.remove("ordering.ascending");
        if (ascending != null) {
            if (getReferenceDescriptor() != null) {
                MWQueryKey queryKey = ((MWRelationalDescriptor) getReferenceDescriptor()).queryKeyNamedIncludingInherited(queryKeyName);
                MWCollectionOrdering ordering = addOrdering(queryKey);
	...
	public static ClassDescriptor legacy45BuildDescriptor() {
		...
		XMLTransformationMapping containerTypeMapping = new XMLTransformationMapping();
		containerTypeMapping.setAttributeName("containerPolicy");
		containerTypeMapping.setAttributeTransformation("legacy45GetContainerPolicyFromRow");
		descriptor.addMapping(containerTypeMapping);

		XMLTransformationMapping usesIndirectionMapping = new XMLTransformationMapping();
		usesIndirectionMapping.setAttributeName("indirectionType");
		usesIndirectionMapping.setAttributeTransformation("legacy45GetIndirectionTypeFromRowForTopLink");
		descriptor.addMapping(usesIndirectionMapping);

        XMLTransformationMapping orderingCriteriaMapping = new XMLTransformationMapping();
        orderingCriteriaMapping.setAttributeName("orderings");
        orderingCriteriaMapping.setAttributeTransformation("legacy45GetOrderingsFromRowForTopLink");
        descriptor.addMapping(orderingCriteriaMapping);
	...
	private MWContainerPolicy legacy45GetContainerPolicyFromRow(Record row) {
		...
		SDKFieldValue keyMethodHandleFieldValue = (SDKFieldValue) row.get("keyMethodHandle");
		Object fieldElement = keyMethodHandleFieldValue.getElements().get(0);
		
		if (fieldElement instanceof Record) {
			Record keyMethodHandleRow = (Record) fieldElement;
			this.legacyValuesMap.put("methodClass", keyMethodHandleRow.get("methodClass"));
			this.legacyValuesMap.put("methodSignature", keyMethodHandleRow.get("methodSignature"));
		}
	...
	public void legacy45PostPostProjectBuild() {
		...
		String containerType = (String) this.legacyValuesMap.remove("collectionType");
		this.containerPolicy.getDefaultingContainerClass().setContainerClass(typeNamed(containerType));
		legacyAdjustContainerTypeAndUsesDefaultContainerClassAttributes(containerType);
	...
	private void legacyAdjustContainerTypeAndUsesDefaultContainerClassAttributes(String containerType) {
		MWClass containerClass = typeNamed(containerType);
		// default is to use Collection container type
		if (containerClass != null) {
            if (containerClass.isAssignableToMap()) {
                this.containerPolicy = new MWMapContainerPolicy(this);
            }
            else if (containerClass.isAssignableToSet()) {
                this.containerPolicy = new MWSetContainerPolicy(this);
            }
            else if (getInstanceVariable().isAssignableToList() && !usesTransparentIndirection()) {
                this.containerPolicy = new MWListContainerPolicy(this);
        ...
        if (containerClass != null) {
            this.containerPolicy.getDefaultingContainerClass().setContainerClass(containerClass);
        }
	...
	private String legacy45GetIndirectionTypeFromRowForTopLink(Record row) {
		...
		if (usesIndirection.equals("true")) {
			return (usesTransparentIndirection.equals("true")) ?
				TRANSPARENT_INDIRECTION
			:
    ...
    private List legacy45GetOrderingsFromRowForTopLink(Record row) {
        List orderings = new Vector();
        SDKFieldValue orderingCriteria = (SDKFieldValue) row.get("orderingCriteria");
       
        String usesOrdering = (String) ((Record) orderingCriteria.getElements().get(0)).get("useOrdering");
        
        if (usesOrdering.equals("true")) {
            String ascending = (String) ((Record) orderingCriteria.getElements().get(0)).get("ascending");           
            String queryKeyName = (String) ((Record) orderingCriteria.getElements().get(0)).get("queryKeyName");
