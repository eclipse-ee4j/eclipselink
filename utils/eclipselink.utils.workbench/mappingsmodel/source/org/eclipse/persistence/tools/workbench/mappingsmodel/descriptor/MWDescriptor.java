...
public abstract class MWDescriptor extends MWModel 
	...
	public static ClassDescriptor legacy50BuildDescriptor() {
		...
		descriptor.setJavaClass(MWDescriptor.class);
		descriptor.setTableName("descriptor");
		descriptor.addPrimaryKeyFieldName("unique-filename");

		descriptor.addDirectMapping("uniqueFilename", "legacyGetUniqueFilenameForTopLink", "legacySetUniqueFilenameForTopLink", "unique-filename");

		InheritancePolicy ip = descriptor.getInheritancePolicy();
		ip.setClassIndicatorFieldName("descriptor-class");
		ip.addClassIndicator(MWTableDescriptor.class, "MWClassDescriptor");
		ip.addClassIndicator(MWAggregateDescriptor.class, "MWAggregateDescriptor");
		ip.addClassIndicator(MWInterfaceDescriptor.class, "MWInterfaceDescriptor");
		ip.addClassIndicator(MWTableDescriptor.class, "MWEjbDescriptor");

		descriptor.addDirectMapping("active", "active");

		DirectToFieldMapping readOnlyMapping = (DirectToFieldMapping) descriptor.addDirectMapping("readOnly", "legacyGetReadOnlyForTopLink", "legacySetReadOnlyForTopLink", "read-only");
		readOnlyMapping.setNullValue(Boolean.FALSE);

		OneToOneMapping bldrClassMapping = new OneToOneMapping();
		bldrClassMapping.setAttributeName("bldrClass");
		bldrClassMapping.setGetMethodName("legacyGetMWClassForTopLink");
		bldrClassMapping.setSetMethodName("legacySetMWClassForTopLink");
		bldrClassMapping.setReferenceClass(MWClass.class);
		bldrClassMapping.setForeignKeyFieldName("bldr-class");
		bldrClassMapping.dontUseIndirection();
		descriptor.addMapping(bldrClassMapping);

		return descriptor;
	}

	private void legacySetMWClassForTopLink(MWClass newValue) {
		this.legacyValuesMap.put("mwClass", newValue);
	}
	private MWClass legacyGetMWClassForTopLink() {
		throw new UnsupportedOperationException();
	}

	private void legacySetUniqueFilenameForTopLink(String newFilename){
		// ignore, this is no longer used
	}
	private String legacyGetUniqueFilenameForTopLink() {
		throw new UnsupportedOperationException();
	}

	private void legacySetReadOnlyForTopLink(boolean readOnly) {
		this.legacyValuesMap.put("readOnly", Boolean.valueOf(readOnly));
	}
	private boolean legacyGetReadOnlyForTopLink() {
		throw new UnsupportedOperationException();
	}
    ...
    protected void legacy50PostBuild(DescriptorEvent event) {
		...
		this.mwClassHandle = new MWClassHandle(this, this.buildMWClassScrubber());
		MWClass mwClass = (MWClass) this.legacyValuesMap.remove("mwClass");
		this.mwClassHandle.setType(mwClass);
		this.name = mwClass.fullName();
	...
	public static ClassDescriptor legacy45BuildDescriptor() {
		...
		descriptor.addPrimaryKeyFieldName("uniqueFilename");
		descriptor.addDirectMapping("uniqueFilename","legacyGetUniqueFilenameForTopLink","legacySetUniqueFilenameForTopLink","uniqueFilename");

		InheritancePolicy ip = descriptor.getInheritancePolicy();
		ip.setClassIndicatorFieldName("descriptorClass");
		ip.readSubclassesOnQueries();
		ip.addClassIndicator(MWTableDescriptor.class, "BldrClassDescriptor");
		ip.addClassIndicator(MWAggregateDescriptor.class, "BldrAggregateDescriptor");
		ip.addClassIndicator(MWInterfaceDescriptor.class, "BldrInterfaceDescriptor");
		ip.addClassIndicator(MWTableDescriptor.class, "BldrEjbDescriptor");

		descriptor.addDirectMapping("active", "active");

		DirectToFieldMapping readOnlyMapping = (DirectToFieldMapping) descriptor.addDirectMapping("readOnly", "legacyGetReadOnlyForTopLink", "legacySetReadOnlyForTopLink", "isReadOnly");
		readOnlyMapping.setNullValue(Boolean.FALSE);
		
		OneToOneMapping classMapping = new OneToOneMapping();
		classMapping.setAttributeName("bldrClass");
		classMapping.setReferenceClass(MWClass.class);
		classMapping.setForeignKeyFieldName("bldrClass");
		classMapping.dontUseIndirection();
		classMapping.setGetMethodName("legacyGetMWClassForTopLink");
		classMapping.setSetMethodName("legacySetMWClassForTopLink");
