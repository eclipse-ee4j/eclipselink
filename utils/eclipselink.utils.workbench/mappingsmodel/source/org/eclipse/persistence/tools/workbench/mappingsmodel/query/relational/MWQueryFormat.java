...
public abstract class MWQueryFormat extends MWModel {
	...
	public static ClassDescriptor legacy45BuildDescriptor() {
		...
		descriptor.descriptorIsAggregate();
		descriptor.setJavaClass(MWQueryFormat.class);
		descriptor.setTableName("QueryFormat");
	
		InheritancePolicy ip = descriptor.getInheritancePolicy();
		ip.setClassIndicatorFieldName("queryFormatClass");
		ip.readSubclassesOnQueries();
		Class[] classes = {
			MWStringQueryFormat.class,
			MWAutoGeneratedQueryFormat.class
		...
		for (int i = 0; i < classes.length; i++) {
			String className = ClassTools.shortNameFor(classes[i]);
			className = "Bldr" + className.substring(2); // 2 for removing MW
			ip.addClassIndicator(classes[i], className);
		}			
	...
	abstract void convertToRuntime(ObjectLevelReadQuery runtimeQuery);
	
	//Conversion from runtime
	abstract void convertFromRuntime(ObjectLevelReadQuery runtimeQuery);
