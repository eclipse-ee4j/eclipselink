...
public final class MWRelationalSpecificQueryOptions 
	...
	void legacySetQueryFormatForToplink(MWQueryFormat queryFormat) {
		this.queryFormat = queryFormat;
	}
	
	void legacySetPrepareForToplink(boolean prepare) {
		this.prepare = prepare;
	}	
	
	void legacySetCacheStatementForToplink(Boolean cacheStatement) {
		this.cacheStatement = new TriStateBoolean(cacheStatement);
	}
	
	void legacySetBindAllParametersForToplink(Boolean bindAllParameters) {
		if (bindAllParameters == null) {
			this.bindAllParameters = new TriStateBoolean(true);
		} else {
			this.bindAllParameters = new TriStateBoolean(bindAllParameters);
		}
	}

	void legacy4XSetQueryFormatForToplink(String legacy4XQueryFormatType, String legacy4XQueryString) {	
		if (legacy4XQueryFormatType.equals("AutoGenerated"))
			this.queryFormat =  new MWAutoGeneratedQueryFormat(this);
		else if (legacy4XQueryFormatType.equals("EJBQL"))
			this.queryFormat =   new MWEJBQLQueryFormat(this, legacy4XQueryString);
		else if (legacy4XQueryFormatType.equals("SQL"))
			this.queryFormat =   new MWSQLQueryFormat(this, legacy4XQueryString);
	}
