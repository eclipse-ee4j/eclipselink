	...
	extends MWModel implements MWModifiable
{
	...
	public static ClassDescriptor legacy50BuildDescriptor() {
		...
		descriptor.setJavaClass(MWMethod.class);
		descriptor.setTableName("method");
	
		descriptor.addDirectMapping("name", "name");
	
		DirectToFieldMapping constructorMapping = (DirectToFieldMapping) descriptor.addDirectMapping("constructor", "constructor");
		constructorMapping.setNullValue(Boolean.FALSE);
		...
		SDKAggregateObjectMapping returnTypeDeclarationMapping = new SDKAggregateObjectMapping();
		returnTypeDeclarationMapping.setAttributeName("returnTypeDeclaration");
		returnTypeDeclarationMapping.setReferenceClass(MWTypeDeclaration.class);
		returnTypeDeclarationMapping.setFieldName("return-type-declaration");
		descriptor.addMapping(returnTypeDeclarationMapping);
	
		SDKAggregateCollectionMapping parameterTypeDeclarationsMapping = new SDKAggregateCollectionMapping();
		parameterTypeDeclarationsMapping.setAttributeName("methodParameters");
		parameterTypeDeclarationsMapping.setGetMethodName("legacy50GetParameterTypeDeclarationsForTopLink" );
		parameterTypeDeclarationsMapping.setSetMethodName("legacy50SetParameterTypeDeclarationsForTopLink" );
		parameterTypeDeclarationsMapping.setReferenceClass(MWTypeDeclaration.class);		
		parameterTypeDeclarationsMapping.setFieldName("parameter-type-declarations");
		descriptor.addMapping(parameterTypeDeclarationsMapping);
	
		SDKObjectCollectionMapping exceptionTypesMapping = MWModel.legacyBuildOneToManyMapping();
		exceptionTypesMapping.setAttributeName("exceptionTypes" );
		exceptionTypesMapping.setGetMethodName("legacy50GetExceptionTypesForTopLink" );
		exceptionTypesMapping.setSetMethodName("legacySetExceptionTypesForTopLink" );
		exceptionTypesMapping.setReferenceClass(MWClass.class);
		exceptionTypesMapping.setFieldName("exception-types" );
		exceptionTypesMapping.setReferenceDataTypeName("exception-type" );
		exceptionTypesMapping.setSourceForeignKeyFieldName("name" );
		exceptionTypesMapping.dontUseIndirection();
		descriptor.addMapping(exceptionTypesMapping);
	
		SDKAggregateObjectMapping accessedAttributeHandleMapping = new SDKAggregateObjectMapping();
		accessedAttributeHandleMapping.setAttributeName("accessedAttributeHandle");
		accessedAttributeHandleMapping.setGetMethodName("getAccessedAttributeHandleForTopLink");
		accessedAttributeHandleMapping.setSetMethodName("setAccessedAttributeHandleForTopLink");
		accessedAttributeHandleMapping.setReferenceClass(MWAttributeHandle.class);
		accessedAttributeHandleMapping.setFieldName("accessed-attribute-handle");
		descriptor.addMapping(accessedAttributeHandleMapping);
	...
	/**
	 * necessary because writes are performed during the read on class stuff in 5.0 class override code.
	 */
	private List legacy50GetParameterTypeDeclarationsForTopLink() {
		Iterator i = new TransformationIterator(methodParameters()) {
			protected Object transform(Object next) {
				return ((MWMethodParameter) next).getTypeDeclaration();
			}
		};
		return CollectionTools.vector(i);
	}
	
	private void legacy50SetParameterTypeDeclarationsForTopLink(List parameterTypeDeclarations) {
		Iterator i = new TransformationIterator(parameterTypeDeclarations.iterator()) {
			protected Object transform(Object next) {
				return new MWMethodParameter(MWMethod.this, (MWTypeDeclaration) next);
			}
		};
		this.methodParameters = CollectionTools.vector(i);
	}
	
	/**
	 * legacy projects did not use MWClassHandle
	 */
	private void legacySetExceptionTypesForTopLink(Collection exceptionTypes) {
		this.exceptionTypeHandles = new Vector();
		for (Iterator stream = exceptionTypes.iterator(); stream.hasNext(); ) {
			this.exceptionTypeHandles.add(new MWClassHandle(this, (MWClass) stream.next(), this.exceptionTypeScrubber()));
		}
	}
	/**
	 * necessary because of the class override trickery, writing is actually done on read
	 */
	private Collection legacy50GetExceptionTypesForTopLink() {
		return CollectionTools.list(this.exceptionTypes());
	}
	...
	public static ClassDescriptor legacy45BuildDescriptor() {
		...
		descriptor.addDirectMapping("name", "name");
		descriptor.addDirectMapping("constructor", "constructor");
		...
		XMLTransformationMapping returnTypeDeclarationMapping = new XMLTransformationMapping();
		returnTypeDeclarationMapping.setAttributeName("returnTypeDeclaration");
		returnTypeDeclarationMapping.setAttributeTransformation("legacy45BuildReturnTypeDeclarationFromRow");
		descriptor.addMapping(returnTypeDeclarationMapping);
	
		XMLTransformationMapping parameterTypeDeclarationsMapping = new XMLTransformationMapping();
		parameterTypeDeclarationsMapping.setAttributeName("methodParameters");
		parameterTypeDeclarationsMapping.setAttributeTransformation("legacy45BuildParameterTypeDeclarationsFromRow");
		descriptor.addMapping(parameterTypeDeclarationsMapping);
	
		SDKObjectCollectionMapping exceptionTypesMapping = MWModel.legacyBuildOneToManyMapping();
		exceptionTypesMapping.setAttributeName("exceptionTypes" );
		exceptionTypesMapping.setGetMethodName("legacyGetExceptionTypesForTopLink" );
		exceptionTypesMapping.setSetMethodName("legacySetExceptionTypesForTopLink" );
		exceptionTypesMapping.setReferenceClass(MWClass.class);
		exceptionTypesMapping.setFieldName("exception-types" );
		exceptionTypesMapping.setReferenceDataTypeName("exception-type" );
	...
	private Collection legacyGetExceptionTypesForTopLink() {
		throw new UnsupportedOperationException();
	}

	// bug 3403187 - return types in previous versions were sometimes null (due to project corruption);
	// had to do a transformation mapping because we have no access to the class repository to
	// get the type java.lang.Object.  ~kfm
	private Object legacy45BuildReturnTypeDeclarationFromRow(Record row, Session session) {
		SDKFieldValue fieldValue = (SDKFieldValue) row.get("returnType");
	
		String constructor = (String)  row.get("constructor");
		if (constructor.equals("true")) {
			return null;
		}
		
		// bug 3403187
		if (fieldValue == null) {
			MWClass type = (MWClass) session.readObject(MWClass.class, new ExpressionBuilder().get("name").equal("java.lang.Object"));
			return new MWTypeDeclaration(this, type);
		}
	
		Vector rows = fieldValue.getElements();
		if (rows.size() != 1) {
			throw new IllegalStateException("Invalid number of rows: " + rows.size());
		}
		if ( ! fieldValue.getElementDataTypeName().equals("ArrayType")) {
			throw new IllegalStateException("Invalid element data type name: " + fieldValue.getElementDataTypeName());
		}
	
		Record nestedRow = (Record) rows.firstElement();
	
		String typeName = (String) nestedRow.get("type");
		if (typeName == null) { //bug #3900164
			typeName = "java.lang.Object";
		}
        int dimensionality = Integer.parseInt((String) nestedRow.get("dimensionality"));

        if (ClassTools.classNamedIsArray(typeName)) {
            dimensionality = ClassTools.arrayDepthForClassNamed(typeName);
            typeName = ClassTools.elementTypeNameForClassNamed(typeName);
        }
		MWClass type = (MWClass) session.readObject(MWClass.class, new ExpressionBuilder().get("name").equal(typeName));
		if (type == null) {
			type = (MWClass) session.readObject(MWClass.class, new ExpressionBuilder().get("name").equal("java.lang.Object"));
		}
	
		return new MWTypeDeclaration(this, type, dimensionality);
	}

	/**
	 * transformation mapping method - used for 4.5 projects
	 */
	private Object legacy45BuildParameterTypeDeclarationsFromRow(Record row, Session session) {
		SDKFieldValue fieldValue = (SDKFieldValue) row.get("parameters");
		if (fieldValue == null) {
			return  new Vector();
		}
	
		if ( ! fieldValue.getElementDataTypeName().equals("MethodParameter")) {
			throw new IllegalStateException("Invalid element data type name: " + fieldValue.getElementDataTypeName());
		}
	
		List result =  new Vector(fieldValue.getElements().size());
		for (Iterator stream = fieldValue.getElements().iterator(); stream.hasNext(); ) {
			Record nestedRow = (Record) stream.next();
			String typeName = (String) nestedRow.get("type");
			if (typeName == null) {
				typeName = "java.lang.Object";
			}
			...
			// ignore "parameterIndex"
			result.add(this.buildMethodParameter(type, dimensionality));
		}
		return result;
	}

	/**
	 * 'accessedAttribute' did not exist in 4.5
	 */
	...
	public void legacy45PostPostProjectBuild() {
		...
		if (!isConstructor()) {
			MWClass type = getReturnType();
			//type org.eclipse.persistence.publicinterface.Descriptor no longer exists
			if (type.fullName().equals("org.eclipse.persistence.publicinterface.Descriptor")) {
				setReturnType(this.typeFor(ClassDescriptor.class));
			}
			if (methodParametersSize() == 1) {
				MWMethodParameter parameter = getMethodParameter();
				MWClass parameterType = parameter.getType();
				if (parameterType.fullName().equals("org.eclipse.persistence.publicinterface.Descriptor")) {
					parameter.setType(this.typeFor(ClassDescriptor.class));
				}
