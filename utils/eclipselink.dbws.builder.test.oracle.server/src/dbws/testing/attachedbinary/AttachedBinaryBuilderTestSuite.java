...
public class AttachedBinaryBuilderTestSuite extends DBWSTestSuite {
	...
	static final String SWAREF_FILE = stageDir + "/swaref.xsd";
    ...
    public static void setUp() {
        ...
        FileWriter swarefFileWriter = null;
        try {
            swarefFileWriter = new FileWriter(SWAREF_FILE);
            swarefFileWriter.write(SWAREF_XSD);
            swarefFileWriter.flush();
        } catch (IOException e) {
            e.printStackTrace();
        } finally {
            if (swarefFileWriter != null) {
                try {
                    swarefFileWriter.close();
                    swarefFileWriter.close();
                } catch (IOException e) {}
            }
        }
    ...
    static final String SWAREF_XSD =
        "<?xml version=\"1.0\" encoding=\"UTF-8\"?>" + 
