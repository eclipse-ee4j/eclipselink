package dbws.testing.simpleplsql.src.test;
...
public class Client {
    ...
    static final String SOAP_SIMPLEPLSQL_REQUEST =
      "<env:Envelope xmlns:env=\"http://schemas.xmlsoap.org/soap/envelope/\">" +
        "<env:Header/>" +
        "<env:Body>" +
          "<testBoolean xmlns=\"urn:simpleplsqlService\">" +
            "<X>1</X>" +
          "</testBoolean>" +
        "</env:Body>" +
      "</env:Envelope>";
    ...
    public static void main(String[] args) throws SOAPException, TransformerException {
        ...
        QName qname = new QName("urn:simpleplsqlService", "simpleplsqlServicePort");
        Service service = Service.create(new QName("urn:simpleplsql", "simpleplsqlService"));
        service.addPort(qname, SOAPBinding.SOAP11HTTP_BINDING,
            "http://" + hostname + ":" + port + "/simpleplsql/simpleplsql");
        Dispatch<SOAPMessage> sourceDispatch = service.createDispatch(qname, SOAPMessage.class,
            Service.Mode.MESSAGE);
        SOAPMessage request = createSOAPMessage(SOAP_SIMPLEPLSQL_REQUEST);
        ...
        if (result != null) {
            ...
            if (!nodeEqual) {
                System.out.println("simpleplsql response not same as control document");
    ...
    static final String SOAP_SIMPLESP_RESPONSE =
        ...
        "<srvc:testBooleanResponse xmlns:srvc=\"urn:simpleplsqlService\">" +
            "<srvc:result>" +
                "<simple-xml-format>" +
                    "<simple-xml>" +
                        "<result>1</result>" +
