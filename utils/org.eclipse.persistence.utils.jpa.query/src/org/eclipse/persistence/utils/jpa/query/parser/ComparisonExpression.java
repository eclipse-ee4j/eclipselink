 ...
 * a primitive Java language type and the other is the wrapped Java class type
 * equivalent (e.g., int and Integer are like types in this sense).
 * <p>
 * There is one exception to this rule: it is valid to compare numeric values
 * for which the rules of numeric promotion apply. Conditional expressions
 * attempting to compare non-like type values are disallowed except for this
 * numeric case.
 * <p>
 * Note that the arithmetic operators and comparison operators are permitted to
 * be applied to state-fields and input parameters of the wrapped Java class
 * equivalents to the primitive numeric Java types. Two entities of the same
 * abstract schema type are equal if and only if they have the same primary key
 * value. Only equality/inequality comparisons over enumeration constants are
