...
public abstract class AbstractExpression extends StringExpression
	...
	private AbstractExpression parse(WordParser wordParser,
		...
		while (!wordParser.isTail()) {
			...
			// Store the child but skip a very special case, which happens when parsing two subqueries
			// in a collection expression. Example: (SELECT ... ), (SELECT ... )
		...
		// Return a collection expression, which wraps the sub-expressions
			...
			expression = new CollectionExpression(this, children, separatedBySpaces, separatedByCommas);
		}
