...
public class ChangeSummaryXSDWithCSonRootUnsetComplexSingleAtRootTest extends ChangeSummaryOnRootTestCases {
    ...
    public ChangeSummaryXSDWithCSonRootUnsetComplexSingleAtRootTest(String name) {
        super(name);
    }

    public static void main(String[] args) {
        String[] arguments = { "-c", "org.eclipse.persistence.testing.sdo.model.changesummary.ChangeSummaryXSDWithCSonRootUnsetComplexSingleAtRootTest" };
    ...
    /*
        protected void verifyAfterLoad(XMLDocument document) {
            ...
            assertCSonRootUnsetComplexSingleAtRoot(true, shipToDO);
        }

        public void testUnsetComplexSingleBelowRoot() {
            // 1. read in model from a populated XML file - via setup()
            // 2. rootObject will be in pre-operation state
            // 3. get references to the original object tree - before operation is performed
            // 4. [perform operation]
            ...
            cs.beginLogging();
            Property containmentProp = shipToDO.getContainmentProperty();
            shipToDO.unset(containmentProp);

            assertCSonRootUnsetComplexSingleAtRoot(false, shipToDO);
        }*/
}