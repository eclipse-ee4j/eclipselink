...
public class XMLUnmarshalTestCases extends OXTestCase {
    ...
    public void testUnmarshalFromInputSourceDOMPlatformJSON(){
        InputStream stream = ClassLoader.getSystemResourceAsStream(XML_RESOURCE);
        InputStream stream2 = ClassLoader.getSystemResourceAsStream(XML_RESOURCE);

    	platform = Platform.DOM;
    	XMLContext ctx = getXMLContext(SESSION_NAME);
        assertEquals("org.eclipse.persistence.oxm.platform.DOMPlatform", ctx.getSession(0).getProject().getDatasourceLogin().getDatasourcePlatform().getClass().getName());
    	
    	XMLUnmarshaller unm = ctx.createUnmarshaller();
        
        
        unm.setMediaType(MediaType.APPLICATION_XML);
        Object unmarshalledObject = unm.unmarshal(stream);
        assertEquals(controlObject, unmarshalledObject);

        unm.setMediaType(MediaType.APPLICATION_JSON);
        try{
           unmarshalledObject = unm.unmarshal(stream2);
        }catch(XMLMarshalException e){
