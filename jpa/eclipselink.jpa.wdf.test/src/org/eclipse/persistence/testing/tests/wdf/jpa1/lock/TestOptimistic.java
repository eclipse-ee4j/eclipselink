...
public class TestOptimistic extends JPA1Base {
	...
	private static final boolean ALLOW_FOR_ECLIPSELINK_OLE = true;  // bug 320643
	...
	public void testOptimisticLockExceptionUpdateUpdate() {
		...
		try {
			...
			EntityManager em2 = env.getEntityManagerFactory()
					.createEntityManager();
			try {
    ...
    private static void assertFailingEntity(Object entity,
            org.eclipse.persistence.exceptions.OptimisticLockException eclipseLinkOle) {
        // $JL-EXC$ expected behavior
        Object failingEntity = eclipseLinkOle.getObject();
        if (failingEntity != null) {
            verify(entity.equals(failingEntity), "wrong entity");
        }
    }
	...
	private static void assertExceptionWrapsOLE(Object entity, PersistenceException rbe) {
		...
		if (cause instanceof org.eclipse.persistence.exceptions.OptimisticLockException) {
            if (ALLOW_FOR_ECLIPSELINK_OLE) {
                // OK
                // ignore asserting correct entity 
            } else {
		        Assert.fail("unexpected EclipseLink OLE");
		    }
	...
	public void testIllegalVersionAccessNew() {
		...
		try {
			...
			closeEntityManager(em);
			verify(caughtException,
					"PersistenceException not thrown for versionModification");
		}
	}
	...
	public void testFlushWithVersion() {
		...
		try {
			...
			verify(rev.getReviewDate().equals(Date.valueOf("2005-12-23")),
					"wrong reviewDate");
	...
	public void testMergeWithVersion() {
		...
		try {
			...
			verify(env.isTransactionMarkedForRollback(em),
					"transaction not marked for rollback on OptimisticLockException");
		} finally {
			closeEntityManager(em);
		}
	}
	...
	public void testIsoLevel() throws SQLException {
		...
		try {
			...
			verify(con.getTransactionIsolation() == Connection.TRANSACTION_READ_COMMITTED,
					"wrong isoLevel");
			int id = 130;
	...
	private void updateNodes(JPAEnvironment env, EntityManager em,
			String newName) {
		env.beginTransaction(em);
