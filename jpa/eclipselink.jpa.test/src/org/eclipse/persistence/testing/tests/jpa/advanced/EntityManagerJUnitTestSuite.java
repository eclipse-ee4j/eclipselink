...
//                                em2.createNativeQuery("UPDATE CMP3_RESPONS SET DESCRIPTION = 'NEW' WHERE EMP_ID = "+id).executeUpdate();
    ...
    public void testReadOnlyTransactionalData(){
        if (isOnServer() || getPlatform().isH2() || getPlatform().isDerby() || getPlatform().isSymfoware()) {
            //H2, Derby and Symfoware will get lock timeouts because the transaction is holding a lock on the table for a change.  Test can not be run against these databases.
            return;
        }
        EntityManager em = createEntityManager();
