...
// SessionManager properties currently not supported for SessionBroker tests.add("testDeadConnectionFailover");
// SessionManager properties currently not supported for SessionBroker tests.add("testDeadPoolFailover");
...
// SessionManager properties currently not supported for SessionBroker  SessionBroker tests.add("testConnectionPolicy");
...
// SessionManager properties currently not supported for SessionBroker tests.add("testPostAcquirePreReleaseEvents_InternalConnectionPool");
// SessionManager properties currently not supported for SessionBroker tests.add("testPostAcquirePreReleaseEvents_ExternalConnectionPool");
...
// SessionManager properties currently not supported for SessionBroker  tests.add("testConnectionPolicySetProperty");
...
// SessionManager properties currently not supported for SessionBroker  tests.add("testNonPooledConnection");
// SessionManager properties currently not supported for SessionBroker tests.add("testExclusiveIsolatedLeaksConnectionOnClear");
// uses another pu - customizeAddTarget            tests.add("testSetTargetQueryOneToMany");
    ...
    /**
     * Bug# 219097 This test would normally pass, but we purposely invoke an
     * SQLException on the firstName field so that we can test that an
     * UnsupportedOperationException is not thrown as part of the roll-back
     * exception handling code for an SQLException.
     */
    public void testForceSQLExceptionFor219097() {
        boolean exceptionThrown = false;
        // Set an immutable properties Map on the em to test addition of
        // properties to this map in the roll-back exception handler
        EntityManager em = createEntityManager(Collections.emptyMap());
        beginTransaction(em);

        Employee emp = new Employee();
        /*
         * Provoke an SQL exception by setting a field with a value greater than
         * field length. 1 - This test will not throw an exception without the
         * Collections$emptyMap() set on the EntityhManager or the exceeded
         * field length set on firstName. 2 - This test will throw an
         * UnsupportedOperationException if the map on AbstractSession is not
         * cloned when immutable - bug fix 3 - This test will throw an
         * SQLException when operating normally due to the field length
         * exception
         */
        ...
        try {
            ...
            if (cause instanceof UnsupportedOperationException) {
                ...
                fail(cause.getClass()
                        + " Exception was thrown in error instead of expected SQLException.");
            } else {
                exceptionThrown = true;
            }
        } finally {
    ...
    public void testRefreshNotManaged() {
        ...
        try {
            ...
            fail("entityManager.refresh(notManagedObject) threw a wrong exception: "
    ...
    public void testEMFClose() {
        ...
        String errorMsg = "";
        EntityManagerFactory em = new EntityManagerFactoryImpl(
                getDatabaseSession());
        try {
            em.close();
    ...
    public void testRefreshOPTIMISTICLock() {
        ...
        if (!isOnServer()) {
            ...
            try {
                ...
                try {
                    Employee employee2 = em2.find(Employee.class,
                            employee.getId());
        ...
        if (!isOnServer() && isSelectForUpateNoWaitSupported()) {
            ...
            try {
                ...
                try {
                    ...
                    em2.refresh(dept2, LockModeType.PESSIMISTIC_READ,
                            properties);
        ...
        if (!isOnServer() && isSelectForUpateNoWaitSupported()) {
            ...
            try {
                ...
                try {
                    ...
                    // According to the spec a 0 indicates a NOWAIT clause.
                    properties.put(QueryHints.PESSIMISTIC_LOCK_TIMEOUT, 0);
                    em2.refresh(dept2, LockModeType.PESSIMISTIC_WRITE,
                            properties);
                } catch (PersistenceException ex) {
                    if (ex instanceof javax.persistence.PessimisticLockException) {
                        pessimisticLockException = ex;
    ...
    // Bug5955326, refresh should invalidate the shared cached object that was
    // deleted outside of JPA.
    public void testRefreshInvalidateDeletedObject() {
        ...
        try {
            ...
            boolean isIsolated = ((EntityManagerImpl) em1).getDatabaseSession()
                    .getClassDescriptorForAlias("Address").isIsolated();
            if (isIsolated) {
            ...
            // Delete Address outside of JPA so that the object still stored in
            // the cache.
    ...
    public void internalTestFlushMode(FlushModeType emFlushMode,
        FlushModeType queryFlushMode) {
    ...
    public void testSetRollbackOnly() {
        ...
        try {
            ...
            /*
             * If JTA case after exception has occured the connection is
             * immediately closed. Attempt to perform read causes a new
             * connection to be acquired. Because the unitOfWork is no longer in
             * transaction the connection is acquired by ServerSession. In case
             * the ServerSession's read connection pool is JTA-managed the
             * acquired connection that attempt fails on some app. servers (WLS)
             * while others (GlassFish) return connection that correspond to no
             * transaction at all (that would cause the old data to be read and
             * the test to fail). Alternatively, if a non-JTA-managed connection
             * pool used for reading then the newly acquired connection is
             * always retuns the old object (and the test would fail, again).
             * Note that in case of internal connection pool the connection is
             * still kept after the exception is thrown and therefore the read
             * returns the new data (the test passes).
             */
            if (!isOnServer()) {
    ...
    public void testSubString() {
        ...
        List employees = em
                .createQuery("SELECT object(e) FROM Employee e where e.firstName = substring(:p1, :p2, :p3)")
                .setParameter("p1", firstName)
                .setParameter("p2", new Integer(firstIndex))
                .setParameter("p3", new Integer(lastIndex)).getResultList();
    ...
    /*
     * public void testNewObjectNotCascadePersist(){ IllegalStateException
     * exception = null; EntityManager em = createEntityManager();
     * beginTransaction(em); Golfer g = new Golfer(); WorldRank wr = new
     * WorldRank(); g.setWorldRank(wr); em.persist(g); try{ em.flush(); }catch
     * (IllegalStateException ex){ exception = ex; }finally{
     * rollbackTransaction(em); closeEntityManager(em); }
     * assertNotNull("Failed to throw IllegalStateException see bug: 237279 ",
     * exception);
     * 
     * }
     */
    ...
    public void testIgnoreRemovedObjectsOnDatabaseSync() {
        ...
        try {
            ...
            for (Iterator<Employee> iterator = emps.iterator(); iterator
                    .hasNext();) {
                em.remove(iterator.next());
            }
        } catch (RuntimeException ex) {
            ...
            try {
                ...
                Runnable runnable = new Runnable() {
                    ...
                    public void run() {
                        ...
                        try {
                            beginTransaction(em2);
                            LargeProject lp2 = em2.find(LargeProject.class,
                                    lp1.getId());
                            HashMap properties = new HashMap();
                            // According to the spec a 0 indicates a NOWAIT
                            // clause.
                            properties.put(QueryHints.PESSIMISTIC_LOCK_TIMEOUT, 0);
                ...
                // t2 should have failed to get a lock with NOWAIT and hence
    ...
    /*
     * Helper class for testPESSIMISTIC_ExtendedScope. Kills transaction that is
    ...
    public void testPESSIMISTIC_ExtendedScope() {
        // Cannot create parallel entity managers in the server. Uses FOR UPDATE
        // clause which SQLServer doesn't support.
        if (isOnServer() || !isSelectForUpateSupported() || !isPessimisticWriteLockSupported()) {
            return;
        }

        // If FOR UPDATE NOWAIT is not supported then FOR UPDATE is used - and
        // that could lock the test (em2) for a long time (depending on db
        // setting).
        boolean shouldSpawnThread = !isSelectForUpateNoWaitSupported();
        // To avoid that a separate thread is spawned that - after waiting the
        // specified time -
        // completes the locking transaction (em1) and therefore clears the way
        // to em2 go ahead.
        ...
        // indicates which method on the first entity manager is used to lock
        // the object.
        String[] testModeArray1 = { "query", "find", "lock", "refresh" };
        // indicates which method on the second entity manager is used to test
        // the lock.
        String[] testModeArray2 = { "query", "find", "update_name",
                "update_salary", "remove_project", "remove_respons",
                "update_project", "update_respons", "lock", "refresh" };
        /*
         * test runs all combinations of elements of the above three arrays. To
         * limit the number of configuration for debugging override these array,
         * for instance: boolean[] isObjectCached = {false}; String[]
         * testModeArray1 = {"lock"}; String[] testModeArray2 = {"find"};
         */
        // testMode1 loop
                    ...
                    try {
                        ...
                        try {
                            ...
                            if (shouldSpawnThread) {
                                // after waiting TransactionKiller rollback em1
                                // transaction unlocking way for em2 to proceed.
                                // the test assumes that em2 waiting for
                                // timeToWait means em2 waiting on the lock
                                // acquired by em1.
                            ...
                            // transaction killed by TransactionKiller is
                            // treated as PessimisticLockException
    ...
    // CTS: AFTER A REMOVE THEN A PERSIST ON THE SAME ENTITY, CONTAINS RETURNS
    // FALSE
    ...
    // doRollback - rollbacks transaction that contains remove and the second
    // persist.
        ...
        for (int i = 0; i < 32; i++) {
            ...
            try {
                if (!exceptionWasThrown) {
                    ...
                    if (doTransaction) {
                        Employee employeeReadFromCache = em.find(
    ...
    public void testPersistManagedException() {
        ...
        // bug240061: cannot operate on a closed/rolledback transaction in JBOSS
        // - JBOSS's proxy throws away the old one and will
        // attempt to get a new EM. An exception is thrown when this new EM
        // tries to register with the transaction
        // emp = em.find(Employee.class, id);
        // em.remove(emp);
        rollbackTransaction(em);
    ...
    // CTS: OBJECTS REMOVED AND THEN FLUSHED ARE RETURNED BY QUERIES AFTER THE
    // FLUSH
    ...
    /**
     * Test that deleting an employee works correctly. This test case was added
     * in 8177 for 324321 and modified for 325605. The issue is that the status
     * enum will be cascade deleted even if it is not instantiated (lazy)
     * because the owning object does not know if the collection is empty
     * without instantiating it. DB2 will therefore emit warning logs that are
     * printed at FINEST in this lazy case. This test is a modification of
     * testDeleteEmployee() that verifies instantiated lists are also ok
     * 
     * 11/17/2010-2.2 Michael O'Brien - 325605: Filter out SQL warnings that are
     * not SQL statements but are logged at a non-warning level. This affects
     * only implementors of SessionLog that perform log diagnostics/tracking in
     * addition to logging.
     */
    public void testDeleteEmployee_with_status_enum_collection_instantiated() {
        ...
        employee.getWorkWeek().add(Employee.Weekdays.TUESDAY);
        // set enums
        employee.setStatus(Employee.EmployeeStatus.PART_TIME); // enum index is
                                                               // 1
        // set enum on 1 of the 2 phones, leave the other Collection of enums
        // unset - but do a later find to instantiate the Collection
        ...
        try {
            ...
            beginTransaction(em);
            employee = em.find(Employee.class, employee.getId());
            // instantiate the empty Collection of enums to verify we do not
            // cascade delete if we "know" the Collection is empty
    ...
    /*
     * public void testDeleteMan() { EntityManager em = createEntityManager();
     * QuerySQLTracker counter = null; try { beginTransaction(em); Man man = new
     * Man(); em.persist(man); Woman woman = new Woman(); em.persist(woman);
     * PartnerLink link = new PartnerLink(); em.persist(link);
     * man.setPartnerLink(link); link.setMan(man); woman.setPartnerLink(link);
     * link.setWoman(woman); commitTransaction(em); closeEntityManager(em);
     * clearCache(); counter = new QuerySQLTracker(getDatabaseSession()); em =
     * createEntityManager(); beginTransaction(em); // Count SQL. man =
     * em.find(Man.class, man.getId()); woman = em.find(Woman.class,
     * woman.getId()); woman.setPartnerLink(null);
     * counter.getSqlStatements().clear(); em.remove(man);
     * commitTransaction(em); if (counter.getSqlStatements().size() > 2) {
     * fail("Only 2 delete should have occured."); } beginTransaction(em);
     * verifyDelete(man); commitTransaction(em); } finally { if (counter !=
     * null) { counter.remove(); } if (isTransactionActive(em)) {
     * rollbackTransaction(em); } closeEntityManager(em); } }
     */
    ...
    // Test using PERSISTENCE_CONTEXT_CLOSE_ON_COMMIT option to avoid the resume
    // on commit.
    ...
    // Test using PERSISTENCE_CONTEXT_PERSIST_ON_COMMIT option to avoid the
    // discover on commit.
    ...
    public void testNoPersistOnCommit() {
        ...
        System.out.println("testNoPersistOnCommit - begin");
        EntityManager em = createEntityManager();
        beginTransaction(em);
        Employee employee = new Employee();
...
//        properties.putAll(JUnitTestCaseHelper.getDatabaseProperties());
    ...
    // gf3596: transactions never release memory until commit, so JVM eventually
    // crashes
    ...
    // (in that case all changes are merges into the shared cache after
    // transaction committed),
    // but the test would fail with FlushClearCache.Drop - that mode just drops
    // em cache
    ...
    public void testClearWithFlush2() {
        ...
        // setup
        // create employee and manager - and then remove them from the shared
        // cache
        EntityManager em = createEntityManager();
        ...
        // remove both manager_NotInCache and employee_NotInCache from the
        // shared cache
        clearCache();
    ...
    public void testExtendedPersistenceContext() {
        ...
        // TODO: make this test use an extended entity manager in the server by
        // creating from the factory and joining transaction.
        if (isOnServer()) {
            return;
        }
        ...
        // setup: make sure no Employee with the specified firstName exists and
        // create the existing employees.
        EntityManager em = createEntityManager();
        beginTransaction(em);
        try {
            Query q = em.createQuery("SELECT e FROM Employee e WHERE e.firstName = '" + firstName + "'");
        ...
        try {
            ...
            // verify objects are correct in the PersistenceContext after
            // transaction
            ...
            Employee empToBeRemovedFound = em.find(Employee.class,
                    empToBeRemoved.getId());
            if (empToBeRemovedFound != null) {
                fail("empToBeRemoved is still in the db after transaction committed");
            }
            Employee empToBeRefreshedFound = em.find(Employee.class,
                    empToBeRefreshed.getId());
            ...
            Employee empToBeMergedFound = em.find(Employee.class,
                    empToBeMerged.getId());
            ...
            // second test
            // without starting transaction persist, remove, refresh, merge for
            // the second time:
            // now return to the original state of the objects:
            // remove empNew, persist empToBeRemoved, set empToBeRefreshed and
            // empToBeMerged the original salary.
            ...
            // alter empToBeRefreshedFound - can't use empToBeRefreshed here
            // because of its older version().
            ...
            // verify objects are correct in the PersistenceContext after the
            ...
            // verify objects are correct in the db after the third transaction
            // rolled back
            clearCache();
    ...
    protected void internalTestReadTransactionIsolation(boolean shouldOriginalBeInParentCache, boolean shouldUpdateAll, boolean shouldRefresh, boolean shouldFlush) {
        ...
        // The cache should be invalidated because the commit should detect a
        // jump in the version number and invalidate the object.
        EntityManager em2 = createEntityManager();
    ...
    public void testFindDeleteAllPersist() {
        ...
        // Because the address VH is not triggered both objects should be
        // invalidated.
        beginTransaction(em);
        try {
        ...
        // we can no longer rely on the query above to clear the Employee from
        // the persistence context.
        // Clearling the context to allow us to proceed.
        em.clear();
        // persist new empWithoutAddress - the one that has been deleted from
        // the db.
        beginTransaction(em);
        try {
     ...
     * located at
     * eclipselink.jpa.test/resource/eclipselink-validation-failed-model/ and
     * included in eclipselink-validation-failed-model.jar
     */
    public void testEMFWrapValidationException() {
        // Comment out because it's not relevant for getDatabaseProperties()
    ...
    /**
     * At the time this test case was added, the problem it was designed to test
     * for would cause a failure during deployement and therefore this tests
     * case would likely always pass if there is a successful deployment. But it
     * is anticipated that that may not always be the case and therefore we are
     * adding a test case
     */
    ...
    public void testGetLockModeType() {
        ...
        if (isSelectForUpateSupported()) {
            ...
            try {
                ...
                // Note: On some databases EclipseLink automatically upgrade
                // LockModeType to PESSIMSITIC_WRITE
    ...
    public void testUnWrapClass() {
        ...
        try {
            ...
            /*
             * Connection conn1; conn1 = em.unwrap(java.sql.Connection.class);
             * Connection conn = uowImpl.getAccessor().getConnection();
             * assertEquals("Does not return underlying connection", conn,
             * conn1);
             */
        } finally {
            rollbackTransaction(em);
    ...
    public void testQueryHints() {
        ...
        // Set a hint first to trigger query clone (because query is accessed
        // before other hints are set).
        ...
        assertTrue("AS_OF_SCN not set.", ((Number) olrQuery.getAsOfClause()
                .getValue()).intValue() == 12345);
    ...
    // t2.F_NAME ASC
    /**
     * This test ensures that the eclipselink.fetch query hint works. It tests
     * two things.
     * 
     * 1. That the joined attribute is properly added to the query 2. That the
     * query will execute
     * 
     * It does not do any verification that the joining feature actually works.
     * That is left for the joining testing to do.
     */
    ...
    /**
     * Test that the proper exception is thrown when an incorrect batch or fetch
     * query hint is set on the queyr.
     */
    ...
    // test for bug 5234283: WRONG =* SQL FOR LEFT JOIN ON DERBY AND DB2
    // PLATFORMS
    ...
    // test for GlassFish bug 711 - throw a descriptive exception when an
    // uninstantiated valueholder is serialized and then accessed
    ...
    // test for bug 5170395: GET THE SEQUENCING EXCEPTION WHEN RUNNING FOR THE
    // FIRST TIME ON A CLEAR SCHEMA
    ...
    // bug gf830 - attempting to merge a removed entity should throw an
    ...
    public void testMergeRemovedObject() {
        ...
        beginTransaction(em);
        em.remove(em.find(Employee.class, emp.getId())); // attempt to remove
                                                         // the Employee
        try {
    ...
    // detach(object) on a removed object does not throw exception. This test
    // only
    ...
    // bug6167431: tests calling merge on a new object puts it in the cache, and
    // that all new objects in the tree get IDs generated
    public void testMergeNewObject() {
        ...
        try {
            ...
            // this.assertTrue("referenced Address doesn't have its ID generated",
            // managedEmp.getAddress().getId()!=0);
        } finally {
            if (isTransactionActive(em)) {
                rollbackTransaction(em);
            }
    ...
    // bug6180972: Tests calling merge on a new Entity that uses int as its ID,
    // verifying it is set and cached correctly
    ...
    // shouldUseSequencing == false indicates that PKs should be explicitly
    ...
    /**
     * GlassFish Bug854, originally calling with null caused a null-pointer
     * exception.
     */
    public void testCreateEntityManagerFactory() {
        if (isOnServer() && isWeavingEnabled()) {
            // Bug 297628 -
            // jpa.advanced.EntityManagerJUnitTestSuite.testCreateEntityManagerFactory
            // failed on WLS for only dynamic weaving
            // Dynamic weaving for Persistence.createEntityManagerFactory can't
            // be done on app. server.
            return;
        }
        EntityManagerFactory factory = null;

        try {
            // First call with correct properties, to ensure test does not
            // corrupt factory.
    ...
    public void testPrimaryKeyUpdate() {
        ...
        try {
            ...
            while (persistenceException != null && !(persistenceException instanceof ValidationException)) {
                // In the server this is always a rollback exception, need to
    ...
    public void testCascadePersistToNonEntitySubclass() {
        ...
        try {
            ...
            fail("Persist operation was not cascaded to related non-entity, thrown: "
                    + e);
        } finally {
    ...
    /**
     * Bug 801 Test to ensure when property access is used and the underlying
     * variable is changed the change is correctly reflected in the database
     * 
     ...
     * variable is changed the change is correctly reflected in the database
     * 
     * In this test we test making the change when an existing object is read
     * into a new EM
     */
...
// DELETE FROM MBR2_PROJ_EMP WHERE EXISTS(SELECT PROJ_ID FROM TL_MBR3_PROJECT WHERE PROJ_ID = MBR2_PROJ_EMP.PROJECTS_PROJ_ID)
    ...
    protected void internalTestUpdateAllProjects(Class cls) {
        ...
        try {
            // setup
            // populate Projects - necessary only if no SmallProject and/or
            // LargeProject objects already exist.
            createProjectsWithName(name, null);
            // save the original names of projects: will set them back in
            // cleanup
            ...
            try {
                ...
                // delete projects that createProjectsWithName has created in
                // setup
    ...
    public void testClosedEmShouldThrowException() {
        ...
        try {
            ...
            errorMsg = errorMsg + "; em.clear() threw wrong exception: "
                    + ex.getMessage();
        }
        try {
            closeEntityManager(em);
    ...
    // gf 1217 - Ensure join table defaults correctly when 'mappedby' not
    // specified
    ...
    public void testMultipleEntityManagerFactories() {
        ...
        try {
            // now try to getEM from the first one - this used to throw
            // exception
            factory1.createEntityManager();
    ...
    // At first tried to use JTATransactionController class, but that introduced
    // dependencies
    // on javax.transaction package (and therefore failed in gf entity
    // persistence tests).
    ...
    /**
     * Test that all of the classes in the advanced model were weaved as
     * expected.
     */
    public void testWeaving() {
        // Only test if weaving was on, test runs without weaving must set this
        // system property.
        if (isWeavingEnabled()) {
             ...
             * field-locking internalTestWeaving(new PlatinumBuyer(), false,
             * false); // field-locking internalTestWeaving(new Department(),
             * true, false); // eager 1-m internalTestWeaving(new Golfer(),
             * true, false); internalTestWeaving(new GolferPK(), true, false);
             */
     ...
     */
    public void internalTestWeaving(Object object, boolean changeTracking,
            boolean indirection) {
    ...
    // This test weaving works with over-writing methods in subclasses, and
    // overloading methods.
    ...
    // This tests that new objects referenced by new objects are found on
    // commit.
    ...
    /**
     * Test that sequence numbers allocated but unused in the transaction kept
     * after transaction commits in case SequencingCallback used (that happens
     * if TableSequence is used without using sequencing connection pool).
     */
    ...
    /**
     * Test getEntityManagerFactory() API tests whether illegalArgumentException
     * is thrown when an entity manager is closed.
     */
    ...
    public void testCollectionAddNewObjectUpdate() {
        ...
        // 264768: Certain application servers like WebSphere 7 will call
        // em.clear() during commit()
        // in order that the entityManager is cleared before returning the em to
        // their server pool.
        // We will want to verify that the entity is managed before accessing
        // its properties and causing
        // the object to be rebuilt with any non-direct fields uninstantiated.
        // Note: even in this case the entity should still be in the shared
        // cache and database below
    ...
    /**
     * See bug# 210280: verify that the URL encoding for spaces and multibyte
     * chars is handled properly in the EMSetup map lookup UC1 - EM has no
     * spaces or multi-byte chars in name or path UC2 - EM has spaces hex(20) in
     * EM name but not in path UC3/4 are fixed by 210280 - the other UC tests
     * are for regression UC3 - EM has spaces in path but not in the EM name UC4
     * - EM has spaces in path and EM name UC5 - EM has multi-byte hex(C3A1)
     * chars in EM name but not in path Keep resource with spaces and multibyte
     * chars separate UC6 - EM has multi-byte chars in path but not EM name UC7
     * - EM has multi-byte chars in path and EM name UC8 - EM has spaces and
     * multi-byte chars in EM name but not in path UC9 - EM has spaces and
    ...
    public void test210280EntityManagerFromPUwithSpaceInNameButNotInPath() {
        // This EM is defined in a persistence.xml that is off
        // eclipselink-advanced-properties (no URL encoded chars in path)
    ...
    public void test210280EntityManagerFromPUwithSpaceInPathButNotInName() {
        // This EM is defined in a persistence.xml that is off [eclipselink-pu
        // with spaces] (with URL encoded chars in path)
        privateTest210280EntityManagerWithPossibleSpacesInPathOrName(//
    ...
    private void privateTest210280EntityManagerWithPossibleSpacesInPathOrName(String puName, String failureMessagePostScript) {
        ...
        try {
            ...
            throw new RuntimeException("A Persistence Unit [" + puName + "] "
                    + failureMessagePostScript, exception);
        } finally {
            if (null != em) {
                closeEntityManager(em);
            }
    ...
    public void internalTestConnectionPolicy(boolean useSetProperty) {
        ...
        try {
            // assume that if JTA is used on server then EntityManager is always
            // injected.
            boolean isEmInjected = isOnServer() && getDatabaseSession().hasExternalTransactionController();
            if (isEmInjected) {
                em = createEntityManager();
                // In server jta case need a transaction - otherwise the wrapped
                // EntityManagerImpl is not kept.
                beginTransaction(em);
    ...
    /*
     * Added for bug 235340 - parameters in named query are not transformed when
     * IN is used. Before the fix this used to throw exception.
     */
    public void testConverterIn() {
        EntityManager em = createEntityManager();
        List<Employee> emps = em.createQuery("SELECT e FROM Employee e WHERE e.gender IN (:GENDER1, :GENDER2)").setParameter("GENDER1", Employee.Gender.Male).setParameter("GENDER2", Employee.Gender.Female).getResultList();
        emps.toString();
        closeEntityManager(em);
    }

    // Bug 237281 - ensure we throw the correct exception when trying to persist
    // a non-entity subclass of an entity
    ...
    // bug 237281 - ensure seeting InheritancePolicy to allow non-entity
    // subclasses to be persisted as their
    // superclass works
    ...
    // bug 232555 - NPE setting embedded attributes after persist in woven
    // classes
    ...
    public void testInheritanceQuery() {
        ...
        // but this is not good, in particular it prevents use of advanced
        // queries with Oraclearrays as bind-variables!!!)
    ...
    // Bug 324406 - Wrong Index in ReportItem when @Embeddable Objects are used
    // in ReportQuery
    ...
    // Bug 332464 - EntityManager running out of connections in exclusive
    // isolated client session mode
    public void testExclusiveIsolatedLeaksConnectionOnClear() {
        ...
        // any query on ExclusiveIsolated session triggers the exclusive
        // connection to be acquired.
        ...
        // before the bug was fixed that used to simply nullify the uow without
        // releasing it and its parent ExclusiveIsolatedSession.
        em.clear();
    ...
    public void internalTestChangeRecordKeepOldValue(boolean addSecondStep) {
        if (isOnServer()) {
            // the test relies upon keeping attached objects between
            // transactions.
            return;
        }
        ...
        try {
            ...
            // persist is not cascaded for department.equipment - have to
            ...
            // persist is not cascaded for department.equipment - have to
            // explicitly persist it.
            // moreover, equipment.id is used as a key in dep.equipment Map,
            // therefore equipmet
            // should be persisted before been added to dep.
            em.persist(equipment2);
            dep.addEquipment(equipment2);
            // persist is not cascaded for employee.department - have to
            // explicitly persist it.
            // moreover, equipment.id is used as a key in dep.equipment Map,
            // therefore equipmet
            // should be persisted before been added to dep.
            ...
            // emp references (directly or through other objects) all the
            // created objects, therefore all of them are copied.
            ...
            // Aggregate
            // emp.setFormerEmployment(new FormerEmployment("New", new
            // EmploymentPeriod(Helper.dateFromYearMonthDate(1989, 0, 1),
            // Helper.dateFromYearMonthDate(1992, 11, 31))));
            ...
            // additional change typically overrides existing aggregate or
            // collection with the new one, then alters aggregate or adds to
            // collection.
            if (addSecondStep) {
                // Aggregate
                ...
                PhoneNumber phone1NewNew = new PhoneNumber("New_New_1", "111",
                        "1111111");
                ...
                // However in theis case we have explicitly persisted the the
                // new member added in thhe current transaction
                // (because Equipment's id is used as its key in the
                // Department.equipment Map)
            ...
            // copies of the updated objects will be altered to contain old
            // values.
            // if altering the test, make sure that emp still references
            // (directly or through other objects) all the updated objects, so
            // that all of them are copied.
            ...
            // before comparison clear all read-only mappings that we didn't set
            // in the attached to em objects.
            for (Object object : updatedObjects) {
                Object readBack = uow2.readObject(object);
                /*
                 * if(readBack instanceof Project) {
                 * ((Project)readBack).getTeamMembers().clear(); } else
    ...
    public void testObjectReferencedInBothEmAndSharedCache_ObjectReferenceMappingVH() {
        ...
        beginTransaction(em);
        em.persist(emp);
        // in JTA case transaction required to obtain ServerSession through
        ...
        // originalObjects should consist of just three objects: emp, emp1,
        // emp2.
        // However if manager_vh is wrapped around manager instance from the
        // shared cache (empShared),
        // the size will be 6: emp, emp1, emp2 and empShared, emp1Shared,
        // emp2Shared.
    ...
    public void testObjectReferencedInBothEmAndSharedCache_AggregateObjectMapping() {
        ...
        // using query by example read empShared corresponding to emp in ghe
        // share cache
