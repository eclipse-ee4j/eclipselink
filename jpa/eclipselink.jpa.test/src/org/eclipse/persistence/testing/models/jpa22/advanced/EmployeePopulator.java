...
public class EmployeePopulator {
    ...
    public StoredProcedureDefinition buildMySQLResultSetProcedure() {
        ...
        proc.addStatement("SELECT E.*, S.* FROM JPA22_EMPLOYEE E, JPA22_SALARY S WHERE E.EMP_ID = S.EMP_ID");
        proc.addStatement("SELECT A.* FROM JPA22_ADDRESS A");
        proc.addStatement("SELECT (t1.BUDGET/t0.PROJ_ID) AS BUDGET_SUM, t0.PROJ_ID, t0.PROJ_TYPE, t0.PROJ_NAME, t0.DESCRIP, t0.LEADER_ID, t0.VERSION, t1.BUDGET, t2.PROJ_ID AS SMALL_ID, t2.PROJ_TYPE AS SMALL_DESCRIM, t2.PROJ_NAME AS SMALL_NAME, t2.DESCRIP AS SMALL_DESCRIPTION, t2.LEADER_ID AS SMALL_TEAMLEAD, t2.VERSION AS SMALL_VERSION FROM JPA22_PROJECT t0, JPA22_PROJECT t2, JPA22_LPROJECT t1 WHERE t1.PROJ_ID = t0.PROJ_ID AND t2.PROJ_TYPE='S'");
        proc.addStatement("SELECT t0.EMP_ID, t0.F_NAME, t0.L_NAME, COUNT(t2.DESCRIPTION) AS R_COUNT FROM JPA22_EMPLOYEE t0, JPA22_RESPONS t2, JPA22_SALARY t1 WHERE ((t1.EMP_ID = t0.EMP_ID) AND (t2.EMP_ID = t0.EMP_ID)) GROUP BY t0.EMP_ID, t0.F_NAME, t0.L_NAME");
    ...
    public StoredProcedureDefinition buildStoredProcedureDeleteAllResponsibilities() {
        ...
        proc.addStatement("DELETE FROM JPA22_RESPONS");
        return proc;
    }
    ...
    public StoredProcedureDefinition buildStoredProcedureReadAddressCity(DatabasePlatform platform) {
        ...
        if (platform.isSQLServer() || platform.isSybase()) {
            ...
            if (platform.isSQLServer()) {
                statement = "SELECT @city_v = CITY FROM JPA22_ADDRESS WHERE ADDRESS_ID = @address_id_v";
            }
            else {
                statement = "SELECT CITY INTO @city_v FROM JPA22_ADDRESS WHERE ADDRESS_ID = @address_id_v";
            }
        } else {
            statement = "SELECT CITY INTO city_v FROM JPA22_ADDRESS WHERE (ADDRESS_ID = address_id_v)";
        }
    ...
    public StoredProcedureDefinition buildStoredProcedureReadAllAddresses() {
        ...
        proc.addStatement("SELECT ADDRESS_ID from JPA22_ADDRESS");
        return proc;
    }
    ...
    public StoredProcedureDefinition buildStoredProcedureReadAllEmployees() {
        ...
        proc.addStatement("SELECT EMP_ID from JPA22_EMPLOYEE");
        return proc;
    }
    ...
    public StoredProcedureDefinition buildStoredProcedureReadFromAddress(DatabasePlatform platform) {
        ...
        if (platform.isSQLServer() || platform.isSybase()) {
            // 260263: SQLServer 2005/2008 requires parameter matching in the select clause for stored procedures
            statement = "SELECT ADDRESS_ID, STREET, CITY, COUNTRY, PROVINCE, P_CODE FROM JPA22_ADDRESS WHERE ADDRESS_ID = @address_id_v";
        } else {
            statement = "SELECT ADDRESS_ID, STREET, CITY, COUNTRY, PROVINCE, P_CODE FROM JPA22_ADDRESS WHERE (ADDRESS_ID = address_id_v)";
        }
    ...
    public StoredProcedureDefinition buildStoredProcedureReadFromAddressMappedNamed(DatabasePlatform platform) {
        ...
        if (platform.isSQLServer() || platform.isSybase()) {
            // 260263: SQLServer 2005/2008 requires parameter matching in the select clause for stored procedures
            statement = "SELECT ADDRESS_ID AS 'address_id_v', STREET AS 'street_v', CITY AS 'city_v', COUNTRY AS 'country_v', PROVINCE AS 'province_v', P_CODE AS 'p_code_v' FROM JPA22_ADDRESS WHERE ADDRESS_ID = @address_id_v";
        } else {
            statement = "SELECT ADDRESS_ID AS 'address_id_v', STREET AS 'street_v', CITY AS 'city_v', COUNTRY AS 'country_v', PROVINCE AS 'province_v', P_CODE AS 'p_code_v' FROM JPA22_ADDRESS WHERE (ADDRESS_ID = address_id_v)";
        }
    ...
    public StoredProcedureDefinition buildStoredProcedureReadFromAddressMappedNumbered(DatabasePlatform platform) {
        ...
        if (platform.isSQLServer() || platform.isSybase()) {
            // 260263: SQLServer 2005/2008 requires parameter matching in the select clause for stored procedures
            statement = "SELECT ADDRESS_ID AS '1', STREET AS '2', CITY AS '3', COUNTRY AS '4', PROVINCE AS '5', P_CODE AS '6' FROM JPA22_ADDRESS WHERE ADDRESS_ID = @ADDRESS_ID_V";
        } else {
            statement = "SELECT ADDRESS_ID AS '1', STREET AS '2', CITY AS '3', COUNTRY AS '4', PROVINCE AS '5', P_CODE AS '6' FROM JPA22_ADDRESS WHERE (ADDRESS_ID = address_id_v)";
        }
    ...
    public StoredProcedureDefinition buildStoredProcedureParameterTest(DatabasePlatform platform) {
        ...
        if (platform.isSQLServer() || platform.isSybase()) {
            // 260263: SQLServer 2005/2008 requires parameter matching in the select clause for stored procedures
            proc.addStatement("SELECT ADDRESS_ID AS '1', STREET AS '2', CITY AS '3', COUNTRY AS '4', PROVINCE AS '5', P_CODE AS '6' FROM JPA22_ADDRESS WHERE ADDRESS_ID = @ADDRESS_ID_V");
            if (platform.isSQLServer()) {
                proc.addStatement("SELECT @employee_count_v = COUNT(E.EMP_ID) FROM JPA22_EMPLOYEE E, JPA22_SALARY S WHERE E.EMP_ID = S.EMP_ID");
            }
            else {
                proc.addStatement("SELECT COUNT(E.EMP_ID) INTO @employee_count_v FROM JPA22_EMPLOYEE E, JPA22_SALARY S WHERE E.EMP_ID = S.EMP_ID");
            }
        } else {
            proc.addStatement("SELECT ADDRESS_ID AS '1', STREET AS '2', CITY AS '3', COUNTRY AS '4', PROVINCE AS '5', P_CODE AS '6' FROM JPA22_ADDRESS WHERE (ADDRESS_ID = address_id_v)");
            proc.addStatement("SELECT COUNT(E.EMP_ID) INTO employee_count_v FROM JPA22_EMPLOYEE E, JPA22_SALARY S WHERE E.EMP_ID = S.EMP_ID");
    ...
    public StoredProcedureDefinition buildStoredProcedureReadUsingNamedRefCursor() {
        ...
        proc.addStatement("OPEN CUR1 FOR Select E.*, S.* from JPA22_EMPLOYEE E, JPA22_SALARY S WHERE E.EMP_ID = S.EMP_ID");
        proc.addStatement("OPEN CUR2 FOR Select a.* from JPA22_ADDRESS a");
    ...
    public StoredProcedureDefinition buildStoredProcedureReadUsingPosRefCursor() {
        ...
        proc.addStatement("OPEN RESULT_CURSOR FOR Select E.*, S.* from JPA22_EMPLOYEE E, JPA22_SALARY S WHERE E.EMP_ID = S.EMP_ID");
    ...
    public StoredProcedureDefinition buildStoredProcedureReadUsingSysCursor() {
        ...
        proc.addStatement("OPEN p_recordset FOR SELECT EMP_ID, F_NAME FROM JPA22_EMPLOYEE WHERE F_NAME = f_name_v ORDER BY EMP_ID");
    ...
    public StoredProcedureDefinition buildStoredProcedureResultSetAndUpdateFromAddress(DatabasePlatform platform) {
        ...
        if (platform.isSQLServer() || platform.isSybase()) {
            // 260263: SQLServer 2005/2008 requires parameter matching in the select clause for stored procedures
            proc.addStatement("SELECT A.* FROM JPA22_ADDRESS A WHERE A.P_CODE = @new_p_code_v");
            proc.addStatement("SELECT E.*, S.* FROM JPA22_EMPLOYEE E, JPA22_SALARY S WHERE E.EMP_ID = S.EMP_ID");
            if (platform.isSQLServer()) {
                proc.addStatement("SELECT @employee_count_v = COUNT(E.EMP_ID) FROM JPA22_EMPLOYEE E, JPA22_SALARY S WHERE E.EMP_ID = S.EMP_ID");
            }
            ...
            proc.addStatement("UPDATE JPA22_ADDRESS SET P_CODE = @new_p_code_v WHERE P_CODE = @old_p_code_v");
        } else {
            proc.addStatement("SELECT A.* FROM JPA22_ADDRESS A WHERE A.P_CODE = new_p_code_v");
            proc.addStatement("SELECT E.*, S.* FROM JPA22_EMPLOYEE E, JPA22_SALARY S WHERE E.EMP_ID = S.EMP_ID");
            proc.addStatement("SELECT COUNT(E.EMP_ID) INTO employee_count_v FROM JPA22_EMPLOYEE E, JPA22_SALARY S WHERE E.EMP_ID = S.EMP_ID");
    ...
    public StoredProcedureDefinition buildStoredProcedureUpdateFromAddress(DatabasePlatform platform) {
        ...
        if (platform.isSQLServer() || platform.isSybase()) {
            // 260263: SQLServer 2005/2008 requires parameter matching in the select clause for stored procedures
            statement = "UPDATE JPA22_ADDRESS SET P_CODE = @new_p_code_v WHERE P_CODE = @old_p_code_v";
    ...
    public Runner runnerExample1(){
        ...
        final Map <String, RunnerVictory> victoriesThisYear = new HashMap<>(3);
        c.set(2014, 4, 18);
        ...
        final Map <String, RunnerVictory> victoriesLastYear = new HashMap<>(2);
        c.set(2013, 4, 12);
