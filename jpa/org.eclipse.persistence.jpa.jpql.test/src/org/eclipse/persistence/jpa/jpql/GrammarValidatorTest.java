...
/**
 * The unit-tests testing {@link GrammarValidator}.
 *
 * @version 2.3
 * @since 2.3
...
public final class GrammarValidatorTest extends AbstractValidatorTest {
	...
	@Override
	AbstractValidator buildValidator(IQuery query) {
		// Use reflection since the validator is package protected and some class loader
		// might prevent us to instantiate it normally
		try {
//			return new GrammarValidator(query);
			Class<?> validatorClass = Class.forName("org.eclipse.persistence.jpa.internal.jpql.GrammarValidator");
			Constructor<?> constructor = validatorClass.getDeclaredConstructor(IQuery.class);
			constructor.setAccessible(true);
			return (AbstractValidator) constructor.newInstance(query);
		}
		catch (Exception e) {
			return null;
		}
...
//	}
	...
	public void test_OrderByItem_InvalidPath() throws Exception {
		...
		if (queryTestHelper().getPersistenceUnit().getPlatform() == IPlatform.JAVA) {
			testHasOnlyOneProblem(
				problems,
