	...
	protected List<String>[] buildResults(String jpqlQuery,
		...
		for (String className : contentAssistProposals.classNames()) {
			...
			while (iter.hasNext()) {
				...
				// The class name is an expected proposal
				if (proposal.equalsIgnoreCase(className)) {
					// Remove it from the list of expected proposals
					iter.remove();
					// Indicate it was removed, if not, we'll add the class name
					// to the list of unexpected proposals
					removed = true;
					break;
				}
			...
			// The class name found by the content assist is not expected
			if (!removed) {
				unexpectedProposals.add(className);
			}
		}
		...
		for (String tableName : contentAssistProposals.tableNames()) {
			...
			// Iterate through the expected proposals and see if the table name is expected
			while (iter.hasNext()) {
				String proposal = iter.next();

				// The table name is an expected proposal
				if (proposal.equalsIgnoreCase(tableName)) {
					// Remove it from the list of expected proposals
					iter.remove();
					// Indicate it was removed, if not, we'll add the table name
					// to the list of unexpected proposals
					removed = true;
					break;
				}
			...
			// The table name found by the content assist is not expected
			if (!removed) {
				unexpectedProposals.add(tableName);
			}
		}
		...
		for (String columnName : contentAssistProposals.columnNames()) {
			...
			// Iterate through the expected proposals and see if the column name is expected
			while (iter.hasNext()) {
				String proposal = iter.next();

				// The column name is an expected proposal
				if (proposal.equalsIgnoreCase(columnName)) {
					// Remove it from the list of expected proposals
					iter.remove();
					// Indicate it was removed, if not, we'll add the column name
					// to the list of unexpected proposals
					removed = true;
					break;
				}
			...
			// The column name found by the content assist is not expected
			if (!removed) {
				unexpectedProposals.add(columnName);
			}
		}
		...
		for (EnumProposals enumProposals : contentAssistProposals.enumConstant()) {
			...
			for (String enumConstantName : enumProposals.enumConstants()) {
				...
				// Iterate through the expected proposals and see if the enum constnat name is expected
				while (iter.hasNext()) {
					String proposal = iter.next();

					// The enum constant name is an expected proposal
					if (proposal.equalsIgnoreCase(enumConstantName)) {
						// Remove it from the list of expected proposals
						iter.remove();
						// Indicate it was removed, if not, we'll add the enum constant name
						// to the list of unexpected proposals
						removed = true;
						break;
					}
				...
				// The enum constant name found by the content assist is not expected
				if (!removed) {
					unexpectedProposals.add(enumConstantName);
				}
			}
		}
