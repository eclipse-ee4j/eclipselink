...
/**
 * This {@link JoinFactory} creates a new {@link Join} or {@link FetchJoin} when the portion of the
 * query to parse starts with <b>JOIN</b> or <b>FETCH JOIN</b>, respectively.
 *
 * @see FetchJoin
 * @see Join
 *
...
final class JoinFactory extends ExpressionFactory {
	...
	AbstractExpression buildExpression(AbstractExpression parent,
		...
		// INNER JOIN and INNER JOIN FETCH
			...
			if (wordParser.startsWith(Expression.JOIN, index)) {
				index += 4;
				index += wordParser.whitespaceCount(index);
			...
			// INNER
				...
				expression = new Join(parent, Expression.INNER);
			}
		}
		else {
			return null;
